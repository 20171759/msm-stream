...
#define blstrs__g2__G2Affine_ZERO ((blstrs__g2__G2Affine_projective){blstrs__fp2__Fp2_ZERO, blstrs__fp2__Fp2_ONE, blstrs__fp2__Fp2_ZERO})
...
typedef struct {
  ...
  blstrs__fp2__Fp2 y;
  blstrs__fp2__Fp2 z;
} blstrs__g2__G2Affine_projective;

// http://www.hyperelliptic.org/EFD/g1p/auto-shortw-jacobian-0.html#doubling-dbl-2009-l
DEVICE blstrs__g2__G2Affine_projective blstrs__g2__G2Affine_double(blstrs__g2__G2Affine_projective inp) {
...
DEVICE blstrs__g2__G2Affine_projective blstrs__g2__G2Affine_add_mixed(blstrs__g2__G2Affine_projective a, blstrs__g2__G2Affine_affine b) {
  const blstrs__fp2__Fp2 local_zero = blstrs__fp2__Fp2_ZERO;
  ...
  blstrs__g2__G2Affine_projective ret;
...
DEVICE blstrs__g2__G2Affine_projective blstrs__g2__G2Affine_add(blstrs__g2__G2Affine_projective a, blstrs__g2__G2Affine_projective b) {
...
//     GLOBAL blstrs__g2__G2Affine_projective *buckets,
//     GLOBAL blstrs__g2__G2Affine_projective *results,
...
//   const blstrs__g2__G2Affine_projective local_zero = blstrs__g2__G2Affine_ZERO;
...
//   blstrs__g2__G2Affine_projective res = blstrs__g2__G2Affine_ZERO;
...
//   //                    (a) + b +
//   //                    ((a) + b) + c
//   blstrs__g2__G2Affine_projective acc = blstrs__g2__G2Affine_ZERO;
...
#define blstrs__g1__G1Affine_ZERO ((blstrs__g1__G1Affine_projective){blstrs__fp__Fp_ZERO, blstrs__fp__Fp_ONE, blstrs__fp__Fp_ZERO})
...
typedef struct {
  ...
  blstrs__fp__Fp y;
  blstrs__fp__Fp z;
} blstrs__g1__G1Affine_projective;

// http://www.hyperelliptic.org/EFD/g1p/auto-shortw-jacobian-0.html#doubling-dbl-2009-l
DEVICE blstrs__g1__G1Affine_projective blstrs__g1__G1Affine_double(blstrs__g1__G1Affine_projective inp) {
...
// http://www.hyperelliptic.org/EFD/g1p/auto-shortw-jacobian-0.html#addition-madd-2007-bl
DEVICE blstrs__g1__G1Affine_projective blstrs__g1__G1Affine_add_mixed(blstrs__g1__G1Affine_projective a, blstrs__g1__G1Affine_affine b) {
  const blstrs__fp__Fp local_zero = blstrs__fp__Fp_ZERO;
  ...
  blstrs__g1__G1Affine_projective ret;
...
// http://www.hyperelliptic.org/EFD/g1p/auto-shortw-jacobian-0.html#addition-add-2007-bl
DEVICE blstrs__g1__G1Affine_projective blstrs__g1__G1Affine_add(blstrs__g1__G1Affine_projective a, blstrs__g1__G1Affine_projective b) {
...
KERNEL void Exps_Handle_new(  //row_nums 一个线程处理多少行的数据。
    GLOBAL blstrs__scalar__Scalar *exps,
    GLOBAL int *Trans,
    uint n,
    uint window_size,
    ...
    if((gid * num_rows) >= n) return;

    uint nstart = gid * num_rows;
    uint nend = min((nstart + num_rows),n);
    ...
    for(uint i = nstart; i < nend; i++){
      ...
      carry = 0;
      //0...num_window - 1
      for(uint j = 1; j < num_windows; j++){ 
        int left = blstrs__scalar__Scalar_BITS - j *  window_size;
        int w = window_size;
   
        temp = blstrs__scalar__Scalar_get_bits(exps[i],left,w);
        temp += carry;
        carry = 0;
        if(temp >= c/2){
          temp -= c;
        ...
        columns = num_windows - j; //在第几列。
        x = columns * n + i;
        Trans[x] = temp;
      }
      
      //最高位
      int w = window_size + ( blstrs__scalar__Scalar_BITS - num_windows *  window_size );
      int left = 0;
      temp = blstrs__scalar__Scalar_get_bits(exps[i],left,w);
      temp += carry;
      Trans[i] = temp;
...
KERNEL void blstrs__g1__G1Affine_multiexp(
  ...
  GLOBAL blstrs__g1__G1Affine_projective *buckets,
  GLOBAL blstrs__g1__G1Affine_projective *results,
  GLOBAL int *exps,
  uint n,
  uint num_groups,
  uint num_windows,
  ...
  if(gid >= num_windows * num_groups) return;
    ...
    // We have (2^window_size - 1) buckets.
  const uint bucket_len = ( ( 1 << (window_size - 1) ) - 1); 

    // Each thread has its own set of buckets in global memory.
  buckets += (bucket_len + 1) * gid;

  const blstrs__g1__G1Affine_projective local_zero = blstrs__g1__G1Affine_ZERO;
  for(uint i = 0; i <= bucket_len; i++) buckets[i] = local_zero;
  ...
  blstrs__g1__G1Affine_projective res = blstrs__g1__G1Affine_ZERO;

  blstrs__g1__G1Affine_affine temp;
  int id = gid % num_windows;//在第几列。
  //id = num_windows - 1 - id;
  id = id * n;

  for(uint i = nstart; i < nend; i++) {        
    // int ind = exps[i * num_windows + id]; 
    int ind = exps[id + i]; 
    temp = bases[i];
    if(ind < 0){
      ind = -ind;
      temp.y = blstrs__fp__Fp_sub(blstrs__fp__Fp_P,temp.y);
    }
    if(ind--) buckets[ind] = blstrs__g1__G1Affine_add_mixed(buckets[ind], temp);
    ...
    //                    (a) + b +
    //                    ((a) + b) + c
  blstrs__g1__G1Affine_projective acc = blstrs__g1__G1Affine_ZERO;
  for(int j = bucket_len; j >= 0; j--) {
...
KERNEL void blstrs__g2__G2Affine_multiexp(
    ...
    blstrs__g2__G2Affine_affine temp;
    int id = gid % num_windows;//在第几列。
    //id = num_windows - 1 - id;
    id = id * n;
    ...
    for(uint i = nstart; i < nend; i++) {        
      ...
      if(ind < 0){
        ind = -ind;
        temp.y.c0 = blstrs__fp__Fp_sub(blstrs__fp__Fp_P,temp.y.c0);
        temp.y.c1 = blstrs__fp__Fp_sub(blstrs__fp__Fp_P,temp.y.c1);
      }
    // if(ind--) buckets[ind] = blstrs__g2__G2Affine_add_mixed(buckets[ind], temp);
     if(ind == 255) buckets[254] = blstrs__g2__G2Affine_add_mixed(buckets[254], temp);
     else if(ind--) buckets[ind] = blstrs__g2__G2Affine_add_mixed(buckets[ind], temp);
    }
...
// int main()
// {
//     blstrs__g1__G1Affine_affine *host_bases;
//     blstrs__scalar__Scalar *host_exps;
//     //分配host端的内存.
//     host_bases = (blstrs__g1__G1Affine_affine *)malloc(sizeof(blstrs__g1__G1Affine_affine) * N);
//     host_exps  = (blstrs__scalar__Scalar *)malloc(sizeof(blstrs__scalar__Scalar) * N);

//     FILE *Fp;
//     // Fp = fopen("/hdd/wangfeng/G1_cuda/g1_msm/g1_point.txt", "r");
//     Fp = fopen("/hdd/wangfeng/Copy/long/src/gpu/multiexp/g1_points.txt", "r");
//     for(int i = 0; i < N; i++)
//     {
//       for(int j = 11; j >= 0; j--)
//       {
...
//         uint temp;
//         fscanf(Fp, "%x", &temp);
//         host_bases[i].y.val[j] = temp;
   
//       }
//     }
//     FILE *Fp2;
//     //Fp2 = fopen("/hdd/wangfeng/G1_cuda/g1_msm/g1_exps.txt", "r");
//     Fp2 = fopen("/hdd/wangfeng/CUDA_MSM/data.txt", "r");
//     for(int i = 0; i < N; i++)
//     {
//         for(int j = 0; j <= 7; j ++)
//            {
//             uint temp;
//             fscanf(Fp2, "%x", &temp);
//             host_exps[i].val[j] = temp;
//               //printf("%08x ",host_exps[i].val[j]);
//            }
//         //    printf("\n");
//     }
    
//    int *temp_res;
//    temp_res = (int *)malloc(sizeof(int) * N * n_windows);
...
//     cudaMalloc((int **)&dev_res,sizeof(int) * N * n_windows);
//     //cudaMemcpy(dev_res,temp_res,sizeof(int) * N *n_windows,cudaMemcpyHostToDevice);


//     int *dev_trans;
//     cudaMalloc((int **)&dev_trans,sizeof(int) * N * n_windows);


//     blstrs__scalar__Scalar *dev_exps;
//     cudaMalloc((blstrs__scalar__Scalar **)&dev_exps,sizeof(blstrs__scalar__Scalar) * N);
//     cudaMemcpy(dev_exps,host_exps,sizeof(blstrs__scalar__Scalar) * N,cudaMemcpyHostToDevice);
...
//     Exps_Handle_new<<<56,128>>>(dev_exps,dev_res,dev_trans,N,w_size,n_windows,row_nums);
...
//     // int *exps;
//     // exps = (int *)malloc(sizeof(int) * N * n_windows);
//     // cudaMemcpy(exps,dev_trans,sizeof(int) * N * n_windows,cudaMemcpyDeviceToHost);

//     // int tot = 0;
//     // for(int i = 0; i < N * n_windows; i++){
//     //   printf("%d ",exps[i]);
//     //   tot++;
//     //   if(tot == N){
//     //     printf("\n");
...
//     // blstrs__g1__G1Affine_affine *dev_bases;
//     // blstrs__g1__G1Affine_projective *dev_buckets;
//     // blstrs__g1__G1Affine_projective *dev_results;

//     // cudaMalloc((blstrs__g1__G1Affine_projective **)&dev_buckets,sizeof(blstrs__g1__G1Affine_projective) * n_groups * n_windows * buc_len);
//     // cudaMalloc((blstrs__g1__G1Affine_projective **)&dev_results,sizeof(blstrs__g1__G1Affine_projective) * n_groups * n_windows);
//     // cudaMalloc((blstrs__g1__G1Affine_affine **)&dev_bases,sizeof(blstrs__g1__G1Affine_affine) * N);

//     // cudaMemcpy(dev_bases,host_bases,sizeof(blstrs__g1__G1Affine_affine) * N,cudaMemcpyHostToDevice);


//   //   int blocksPerGrid =(n_windows * n_groups + LOCAL_WORK_SIZE - 1) / LOCAL_WORK_SIZE;

//   //   cudaEvent_t start, stop;
//   //   cudaEventCreate(&start);
//   //   cudaEventCreate(&stop);
//   //   cudaEventRecord(start,0);
//   //   blstrs__g1__G1Affine_multiexp<<<blocksPerGrid,LOCAL_WORK_SIZE>>>(dev_bases,dev_buckets,dev_results,dev_res,N,n_groups,n_windows,w_size);
//   //   cudaEventRecord(stop,0);
//   //   cudaEventSynchronize(stop);
//   //   float time;
//   //   cudaEventElapsedTime(&time, start, stop);
...
//   //   blstrs__g1__G1Affine_projective *host_results;
//   //   host_results = (blstrs__g1__G1Affine_projective *)malloc(sizeof(blstrs__g1__G1Affine_projective) * n_groups * n_windows);


//   //   cudaMemcpy(host_results,dev_results,sizeof(blstrs__g1__G1Affine_projective) * n_groups * n_windows,cudaMemcpyDeviceToHost);

//   //   for(int i = 0;i < n_windows;i++){

//   //     printf("%08x ",host_results[i].x.val[0]);
//   //     printf("%08x ",host_results[i].x.val[1]);
//   //     printf("%08x ",host_results[i].x.val[2]);
//   //     printf("%08x ",host_results[i].x.val[3]);
//   //     printf("%08x ",host_results[i].x.val[4]);
//   //     printf("%08x ",host_results[i].x.val[5]);
//   //     printf("%08x ",host_results[i].x.val[6]);
//   //     printf("%08x ",host_results[i].x.val[7]);
//   //     printf("%08x ",host_results[i].x.val[8]);
//   //     printf("%08x ",host_results[i].x.val[9]);
//   //     printf("%08x ",host_results[i].x.val[10]);
//   //     printf("%08x ",host_results[i].x.val[11]);
//   //     printf("\n");
//   // }

// }

//nvcc msm_all_2.cu --optimize=6 --fatbin --gpu-architecture=sm_86 --generate-code=arch=compute_86,code=sm_86 --generate-code=arch=compute_80,code=sm_80 --generate-code=arch=compute_75,code=sm_75 --output-file msm_all_2_4.fatbin